 ...
 */
Cam.prototype.putSystemDateAndTime = function(callback) {
	// Send Response xml
	const options = {
		dateTimeType: 'Manual',
		dateTime: new Date().toLocaleString('en-US', {timeZone: 'Asia/Seoul'}),
		daylightSavings: false,
		timezone: 'KoreaStandardTime-9',
		body: ''
	};
	//make xml body
	options.body = this._envelopeHeaderR() +
		...
		'</tt:DaylightSavings>' +
		(options.timezone !== undefined ? '<tt:TimeZone>' +
			'<tt:TZ xmlns="http://www.onvif.org/ver10/schema">' +
			options.timezone +
			'</tt:TZ>' +
			'</tt:TimeZone>' : '') +
		// ( options.dateTime !== undefined && options.dateTime.getDate instanceof Date ?
		(options.dateTime !== undefined && options.dateTime instanceof Date ? '<tt:UTCDateTime>' +
			'<tt:Time xmlns="http://www.onvif.org/ver10/schema">' +
			'<tt:Hour>' + options.dateTime.getUTCHours() + '</tt:Hour>' +
			'<tt:Minute>' + options.dateTime.getUTCMinutes() + '</tt:Minute>' +
			'<tt:Second>' + options.dateTime.getUTCSeconds() + '</tt:Second>' +
			'</tt:Time>' +
			'<tt:Date xmlns="http://www.onvif.org/ver10/schema">' +
			'<tt:Year>' + options.dateTime.getUTCFullYear() + '</tt:Year>' +
			'<tt:Month>' + (options.dateTime.getUTCMonth() + 1) + '</tt:Month>' +
			'<tt:Day>' + options.dateTime.getUTCDate() + '</tt:Day>' +
			'</tt:Date>' +
			'</tt:UTCDateTime>' : '') +
		'</tds:SystemDateAndTime>' +
		'</tds:GetSystemDateAndTimeResponse>' +
		this._envelopeFooterR();
	
	//Log out
	//console.log('putSystemDateAndTime: ', options.dateTime);
	//console.log('putSystemDateAndTime(options): ', options);
	//console.log('putSystemDateAndTime(body): ', options.body);
	callback.call(this, 0, options.dateTime, options.body);
};
...
/**
 * Envelope footer for all SOAP response messages
 * @returns {string}
 * @private
 */
Cam.prototype._envelopeFooterR = function() {
